---
title: "Using Geographically Weighted Regression to investigate impact of socio-economic factors on wellbeing in London"
...
readr and read_xl are for importing csv and excel spreadsheets into R.

rgdal is R's interface to the "Geospatial Abstraction Library (GDAL)" which is used by other open source GIS packages such as QGIS and enables R to handle a broader range of spatial data formats. regression.
reshape for performing dcast and melt operations on dataframes (converting from long to wide format and vice versa)

Tidyr for tidying the data using - gather(), spread(), unite(), seperate() functions 

The "dplyr" library in particular offers numerous functions for working with R Data Frames. The %>% piping operator, is provided through the magrittr package; this allows better structuring and nesting of calls to "dplyr". data.table is alternative to dplyr for larger datasets and is much faster (providing the same functionality)

GWmodel for geographically weighted regression.library cluster for doing clustering anaysis (kmeans and heirarchial clustering) and caret is the machine learning package. library car for some regression analysis and calculating VIF scores.

Charts are created using ggplot2 and maps using tmap. There are similarities in the syntax used by these two packages. The general approach is of treating graphical elements separately and building the features in a series of layers. 
ggvis is an extension to ggplot for interactive visualisations. Cartogram creates cartograms. RcolorBrewer/colorspace for acccessing colorcschemes for maps (and other graphics). library scales provide methods for automatically determining breaks and labels for axes and legends.
...
setwd("C:/Users/ryank/Documents/City_University/Visual_Analytics/Coursework/Individual_coursework_visualanalytics/London_wellbeing_scores/")
...
factors_wards$WardCode <- as.character(factors_wards$WardCode)
...
Alright, so we have created our first ggplot. We started by passing a data.frame to the function ggplot. From there, we added some aesthetic mappings. x and y are fairly self-explainatory, group = group simply identifies the groups of coordinaites that pertain to individual polygons and fill = Wellbeing2013 will attempt to assign an appropriate color scale to data based on the "Wellbeing2013" column. Next, we added several "geoms" including polygons. The polygons are the brightly colored shapes you see. We add the x and y labels for the plot and name the colorbar Wellbeing2013. Scale_fill_gradientn() changes the properties of the colors displayed and theme() can be used to change a number of properties...in this case the aspect ratio;  Lastly, if required coord_equal() fixes the aspect ratio between the horizontal and vertical scales.
...
ggcorr(label = TRUE, nbreaks=5, hjust = 1, size = 3, palette = "RdYlBu",layout.exp = 10, label_alpha=0.4)
